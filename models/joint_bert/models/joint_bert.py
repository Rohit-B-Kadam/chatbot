...
from models.joint_bert.models.nlu_model import NLUModel
...
class JointBertModel(NLUModel):
    ...
    def __init__(self, slots_num, intents_num, bert_hub_path, num_bert_fine_tune_layers=10, is_bert=True):
        super().__init__()
    ...
    # refer https://tfhub.dev/tensorflow/bert_en_uncased_L-12_H-768_A-12/2
    def build_model(self):
        in_id = Input(shape=(None,), name='input_word_ids', dtype=tf.int32)     # input_word_id
        in_mask = Input(shape=(None,), name='input_mask', dtype=tf.int32)
        in_segment = Input(shape=(None,), name='input_type_ids', dtype=tf.int32)
        in_valid_positions = Input(shape=(None, self.slots_num), name='valid_positions')    # input_valid_position
    ...
    def compile_model(self):
        # Instead of `using categorical_crossentropy` which expect one-hot labels,
    ...
    @staticmethod
    def load(load_folder_path: str, **kwargs):
