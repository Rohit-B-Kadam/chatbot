...
from models.joint_bert.layers.crf_layer import CRFLayer
...
class JointBertCRFModel(JointBertModel):
    ...
    def build_model(self):
        ...
        inputs = bert_inputs + [in_valid_positions, sequence_lengths]
    ...
    def predict_slots_intent(self, x, slots_vectorizer, intent_vectorizer, remove_start_end=True):
        ...
        intents = np.array(
            [intent_vectorizer.inverse_transform([np.argmax(y_intent[i])])[0] for i in range(y_intent.shape[0])])
